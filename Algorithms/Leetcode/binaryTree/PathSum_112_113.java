...
public class PathSum_112_113 {
	...
	public boolean hasPathSumIterative(TreeNode root, int sum) {
		...
		while (cur != null || !stack.isEmpty()) {
			while (cur != null) {		// scan until reaching left bottom leaf
				stack.push(cur);
				sum2 += cur.val;
				cur = cur.left;
			}
			...
			if (cur.right == null && sum2 == sum)		// find a path
				return true;
	...
	public void pathSum(TreeNode root, int sum, List<Integer> tmp, List<List<Integer>> res) {
		...
		tmp.add(root.val);	// 添加进当前集合
		
		if (root.left == null && root.right == null && sum == root.val) {	// find one path
			res.add(new LinkedList<>(tmp));
			tmp.remove(tmp.size() - 1);	// don't forget to remove the last integer
			return;
		} else {	
